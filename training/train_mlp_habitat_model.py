...
with open('configs/best_mlp.json','r') as fp:
...
model_folder = 'habitat_models/global/full/mlp/'
...
if __name__ == "__main__":    
    ...
    parser = argparse.ArgumentParser("Train global habitat model using MLP", add_help=True)
    parser.add_argument(
        "--fold",
        type=int,
        help="Fold",
    ...
    if os.path.exists('%s/classifiers/%s.pth'%(model_folder,model_name)):
        ...
        mlp_hdm = NeuralHDM(archi='mlp',model_name=model_name, problem='all', param_dict=param_dict, k_list=[3,5,10])
